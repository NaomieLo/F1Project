...
#     """
#     Trains a RandomForestRegressor model and saves it.
#     Handles basic one-hot encoding for any categorical features and imputes NaNs.
#     """
#     if "Driver" not in df.columns or "position" not in df.columns:
#         print("Error: DataFrame must contain 'Driver' and 'position' columns.")
#         return None
...
#     # --- Imputation for remaining NaNs in numeric columns (if any) ---
#     # RandomForestRegressor cannot handle NaNs. Impute with median.
#     # Use np.number directly
#     numeric_cols_with_na = (
#         X.select_dtypes(include=np.number).isnull().any()
#     )  # <<< CORRECTED LINE
#     cols_to_impute = numeric_cols_with_na[numeric_cols_with_na].index
#     if not cols_to_impute.empty:
#         print(
...
def train_model(df: pd.DataFrame, model_path: str = "f1_race_model.pkl"):
    """
    Trains a RandomForestRegressor model and saves it as a bundle
    (model + feature list).
    Handles basic one-hot encoding for any categorical features and imputes NaNs.
    """
    if "Driver" not in df.columns or "position" not in df.columns:
    ...
    model = RandomForestRegressor(n_estimators=100, random_state=42, n_jobs=-1)
    print("Training model...")
    # X_train is now fully preprocessed and contains the exact features used for training
    model.fit(X_train, y_train)
    ...
    # ---- MODIFICATION: Save model and feature list as a bundle ----
    model_training_features = (
        X_train.columns.tolist()
    )  # Get feature names AFTER all preprocessing on X
    model_bundle = {"model": model, "features": model_training_features}

    joblib.dump(model_bundle, model_path)  # Save the dictionary bundle
