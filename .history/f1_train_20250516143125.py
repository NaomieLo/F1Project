...
def prepare_dataset(year: int, gp: str):
    ...
    if q_results is not None and not q_results.empty:
        ...
        q_positions.rename(columns={'Abbreviation': 'Driver', 
                                    'Position': 'QualifyingPosition'}, inplace=True)
        q_positions['QualifyingPosition'] = pd.to_numeric(q_positions['QualifyingPosition'], errors='coerce')
        ...
        feats['QualifyingPosition'].fillna(20, inplace=True) # Or a higher number like 24, or median/mean
    else:
        print(f"Warning: Could not load qualifying results for {year} {gp} to get QualifyingPosition. Using default.")
        feats = feats_from_laps.copy() # Important to copy if modifying
        feats['QualifyingPosition'] = 20 # Default value
except Exception as e:
    print(f"Error loading qualifying session results for {year} {gp}: {e}. Using default QualifyingPosition.")
    feats = feats_from_laps.copy() # Important to copy if modifying
    feats['QualifyingPosition'] = 20 # Default value
