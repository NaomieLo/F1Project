...
# In f1_train.py
import fastf1  # Make sure fastf1 is imported if not already at the top
from fastf1 import Cache  # Ensure Cache is available
import pandas as pd
from f1_data import setup_fastf1, fetch_session_data
from f1_features import engineer_features
...
def prepare_dataset(year: int, gp: str):
    ...
    if actual_results is None or actual_results.empty:
        print(
            f"Warning: Could not load actual race results for {year} {gp} to create target variable."
        )
        return pd.DataFrame()
