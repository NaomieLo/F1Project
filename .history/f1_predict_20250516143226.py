...
def predict_race(
    ...
    if q_results is not None and not q_results.empty:
        ...
        print(f"Warning: Could not load Q results for {year} {gp} (prediction). Using default QPos.")
        feats = feats_from_laps.copy()
        feats['QualifyingPosition'] = 20 
except Exception as e:
    print(f"Error loading Q results for {year} {gp} (prediction): {e}. Using default QPos.")
    feats = feats_from_laps.copy()
    feats['QualifyingPosition'] = 20
    if feats.empty:
        print(f"Feature engineering failed or produced no data for {gp}, {year}.")
